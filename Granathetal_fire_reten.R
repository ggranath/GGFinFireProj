...
# Granath et al. Trade-offs in berry production and biodiversity under prescribed burning 
#                and retention regimes in Boreal forests
# Journal of Applied Ecology
# Contact: Gustaf.Granath@gmail.com 
#################################################################################################

# data files needed
# Granathetal_fire_reten_fruitcover.csv
# Granathetal_richness.csv
# Granathetal_vegcover.csv
...
library(gridExtra)
library(grid)
# tested with  MCMCglmm_2.22.1, boot_1.3-18, AICcmodavg_2.1-0, nlme_3.1-128, 
#              ggplot2_2.2.1, gridExtra_2.2.1, dplyr_0.7.2 

# Effects on bilberry/cowberry: fix data, Fig 1 and overall means ####

# load fruit (berry) and plant cover data
zero.dat <- read.csv("Granathetal_fire_reten_fruitcover.csv", stringsAsFactors = FALSE)
...
berry <- aggregate(cbind(VV_fruit, VM_fruit,  VV_cover, VM_cover)~ site + fire + retention, dd, mean)
# berry means
berry.means <- aggregate(cbind(VV_fruit, VM_fruit, VV_cover, VM_cover) ~ retention + fire, berry, mean)
berry.sd <- aggregate(cbind(VV_fruit, VM_fruit, VV_cover, VM_cover) ~ retention + fire, berry, sd)

# Calculate fruits per 1 m2 (sample plot size 0.16 m2)
# unlogged, unburned treatment
berry.means[3, c(3,4)] * (1/0.16)
# Tabel S2, all treatments
sup.tabs2 <- data.frame(berry.means[,1:2], round(berry.means[, c(3:4)] * (1/0.16), digits = 2),
                        round(berry.sd[, c(3:4)] * (1/0.16), digits = 2))

# Simple test if pre-exp cover should be used as a covariate ####
veg <- read.csv("Granathetal_vegcover.csv")
...
prior = list( R = list(V = diag(2), nu = 0.002, fix = 2), 
              G=list(G1=list(V=diag(2)*c(1,0.001), nu=0.002, fix=2), #alt. alpha.mu=0, alpha.V=625^2
...
summary(zip.cow)
# Gives a warning that can be ignored. Contrasts are not straight forward to interpret.
...
# compare burned management treatments, logged(clearcuts) vs retention vs unlogged
# i) clearcut vs retention
clVSret.ci <- zip.cow$Sol[,4] + zip.cow$Sol[,7] + # retention flat burned
  0.5*(zip.cow$Sol[,6] + zip.cow$Sol[,9] + zip.cow$Sol[,10] +
         zip.cow$Sol[,12]) - # near stumps retention. Times 0.5 to get the mean over both microhabitats
...
# Plot sub-full (retained tree patches vs uncut) model
...
# make sure that all models are stored
...
png("figure3_logg_full_comp_fruits.png", width=32, height=30, units="cm", res=300)
...
# Fruit effect plots ####
# requires that modeles have been run already
...
# Figure 3 in Supplemental Material, SM3.
...
png("figureSM3_logg_full_comp_cov.png", width=30, height=30, units="cm", res=300)
grid.arrange(sm2.g1, sm2.g2, sm2.g3, sm2.g4,
             ncol=2, nrow =2)
dev.off()


# Analyse multifunctionality ####
...
# berry means
berry <- cbind(VVberry, VMberry$VM_fruit); colnames(berry)[5]= "VM_fruit"
berry.means <- aggregate(cbind(VV_fruit, VM_fruit) ~ retention + fire, berry, mean)
berry.means$VV_fruit.sd <- aggregate(VV_fruit ~ retention + fire, berry, sd)$VV_fruit
berry.means$VM_fruit.sd <- aggregate(VM_fruit ~ retention + fire, berry, sd)$VM_fruit
berry.means <- cbind(berry.means[,1:2], berry.means[,3:6] *(1/0.16))
...
rich <- read.csv("Granathetal_richness.csv")

# pollinators
# richness means
poll.rich.means <- aggregate(pollinators_sr ~ retention + fire, rich, mean)
poll.rich.means$pollinators_sr.sd <- aggregate(pollinators_sr ~ retention + fire, rich, sd)$pollinators_sr

# stats models
# gls pollinators
mod.poll.sr <- gls(pollinators_sr ~ retention*fire, rich,
                   weights=varIdent(form=~1|retention*fire))
anova(mod.poll.sr)
summary(mod.poll.sr)
plot(mod.poll.sr)
...
# gamma pollinators
mod.poll.sr.gamma = glm(pollinators_sr ~ retention*fire, rich, family=Gamma(link = log) )
...
new_dat <- unique(rich[, c("fire", "retention")])
new_dat <- new_dat[order(new_dat$fire, new_dat$retention),]
...
# saproxylic beetles
# richness means
beetle.rich.means <- aggregate(sap_beetles_sr ~ retention + fire, rich, mean)
beetle.rich.means$sap_beetles_sr.sd <- aggregate(sap_beetles_sr ~ retention + fire, rich, sd)$sap_beetles_sr

# stats models
# gls beetle
mod.beetle.sr <- gls(sap_beetles_sr ~ retention*fire, rich,
                     weights=varIdent(form=~1|retention*fire))
anova(mod.beetle.sr)
summary(mod.beetle.sr)
plot(mod.beetle.sr)

# gamma beetle
mod.beetle.sr.gamma = glm(sap_beetles_sr ~ retention*fire, rich, family=Gamma(link = log) )
summary(mod.beetle.sr.gamma)
plot(mod.beetle.sr.gamma)
leuk.diag <- glm.diag(mod.beetle.sr.gamma)
glm.diag.plots(mod.beetle.sr.gamma, leuk.diag)

# predict beetle changes
p.beetle.gls <- glm.pred(mod=mod.beetle.sr, new_dat=new_dat)
p.beetle.gamma <- glm.pred(mod=mod.beetle.sr.gamma, new_dat=new_dat)

# vegetation
# richness means
plant.rich.means <- aggregate(cbind(plant_sr, bryo_sr, lichen_sr) ~ retention + fire, rich, mean)
...
# stats models
# gls plant
mod.plant.sr <- gls(plant_sr ~ retention*fire, rich,
                    weights=varIdent(form=~1|retention*fire))
anova(mod.plant.sr)
...
#gamma plant
mod.plant.sr.gamma = glm(plant_sr ~ retention*fire, rich, family=Gamma(link = log) )
...
# gls bryo
mod.bryo.sr <- gls(bryo_sr ~ retention*fire, rich,
                   weights=varIdent(form=~1|retention*fire))
anova(mod.bryo.sr)
summary(mod.bryo.sr)
plot(mod.bryo.sr)
...
#gamma bryo
mod.bryo.sr.gamma = glm(bryo_sr ~ retention*fire, rich, family=Gamma(link = log) )
...
# gls lichen
mod.lichen.sr <- gls(lichen_sr ~ retention*fire, rich,
                     weights=varIdent(form=~1|retention*fire))
anova(mod.lichen.sr)
...
#gamma lichen
mod.lichen.sr.gamma = glm(lichen_sr ~ retention*fire, rich, family=Gamma(link = log) )
...
multi.table <- cbind(p.VVberry.gamma, p.VMberry.gamma, p.poll.gamma,
                     p.beetle.gamma, p.plant.gamma, p.bryo.gamma, p.lichen.gamma)
...
means_richness <- cbind(berry.means, poll.rich.means[, -c(1:2)], 
...
# model checking stuff that can be used on the data and produced models
...
# adapted from
# https://rubenarslan.github.io/paternal_age_fitness/0__helpers.html
plot_zero_infl = function(variable) {
  ...
  if( var(variable,na.rm=T) > mean(variable,na.rm=T)) { 
    lab = paste0("Overdispersed (d=", 
                 ...
                 ") poisson generated using \ndistribution mean and variance via neg.bin") 
  ...
  qplot(rpois.od(n=NROW(variable),mean(variable,na.rm=T), 
    ...
    ggtitle(paste0("Gray = observations; Orange = generated\n",lab)) + 
...
ranef.mcmc3 <- colMeans(zip.cow$Liab) - predict(zip.cow, 
                                                marginal = NULL, type = "terms")
hist(ranef.mcmc3)
qqnorm(ranef.mcmc3) ; qqline(ranef.mcmc3)

# Pre-experiment vegetation: distribution, nmds ####
# test for using pre-exp cover of shrubs as covariate can be found
# under "Experiment effects - bilberry/cowberry"
veg <- read.csv("Granathetal_vegcover.csv")
veg <- veg[!(veg$retention == "ten"),] # remove lower retention level
veg <- veg[veg$year == "y2000"| veg$year == "y2011",] # select pre-experiment and 2011 data

# check bilberry and cowberry start values
veg.com <- veg.com[veg.com$year == "y2000",]
...
library(vegan)
veg.com <- veg[veg$year == "y2000", 6:ncol(veg)] # select species
